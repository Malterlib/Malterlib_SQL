...
namespace NMib
{
    namespace NSQL
	{
		...
		CTransactionResult *CSQLConnection::fp_CommitTransaction(CTransaction *_pTransaction, CDatabaseImplementation *_pImp)
		{
			...
			NMem::fg_ObjectSet(pResult->m_Results.f_GetArray(), (CQueryResult *)nullptr, _pTransaction->m_nTransactions);
			int iResult = 0;
		...
		bint CSQLConnection::f_Create(const ch8 *_pImplementation, const NRegistry::CRegistry_CStr &_Parameters, mint _nWorkerThreads)
		{
			if (mp_pMainImp)
			{
